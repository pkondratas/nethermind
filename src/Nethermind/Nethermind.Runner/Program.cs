...
async Task<int> ConfigureAsync(string[] args)
{
    ...
    {
        (args, grandineConfig) = Grandine.ConfigureGrandine(args);
    }
...
void AddGrandineNetworkAndRpcConfig(Dictionary<string, List<string>> grandineConfig, IConfigProvider configProvider, ParseResult parseResult)
{
...
static class BasicOptions
{
    ...
    public static CliOption<string> Grandine { get; } =
        new("--grandine")
        {
            Description = "Grandine Consensus Client configuration",
            HelpName = "grandine"
        };
}
